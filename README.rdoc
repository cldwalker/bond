...
Bond can autocomplete argument(s) to methods, uniquely completing per module and per method. Bond brings irb's completion
...
To start off, replace irb's completion (irb/completion) with Bond's enhanced version in your irbrc :

  require 'bond'
  Bond.start

This gives you more consistent method completion on any objects, file completion of strings
